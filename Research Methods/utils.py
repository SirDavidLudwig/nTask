...
def train(title, agent, *args, maxError = 0, maxEpochs = 0, epsilon=0, simLimit=0, errorCheck=50, useBest = False, restoreBest = False, **kwargs):
    s = time.time()
    i = totalSteps = 0
    bestError = error = agent.error() if agent.error else -1
    bestEpoch = 0
    try:
        while error >= maxError and (not maxEpochs or agent.epoch() < maxEpochs):
            ...
            i += 1
            if i % errorCheck == 0:
                error = agent.error() if agent.error else -1
