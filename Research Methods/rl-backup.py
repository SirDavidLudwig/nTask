...
#     def __init__(self, nInputs, nOutputs, verbose = False):
#         # Long-term memory
#         self.__ltm = LTM(nInputs)
        
#         # Neural network
#         self.__model = keras.models.Sequential()
#         self.__model.add(keras.layers.Dense(nOutputs, use_bias=False, input_shape=(nInputs,), activation="linear"))
#         self.__model.compile(loss=keras.losses.mean_squared_error, optimizer="adam")
...
#         values = self.__model.predict(inputs, verbose = self.__verbose)
#         self.__lastPrediction = (inputs, values)
#         return values
    
#     def value(self, state, action):
#         inputs = np.array([self.encode(f"{state}*{action}")])
#         values = self.__model.predict(inputs, verbose = self.__verbose)[0]
#         return values
...
#         self.__model.fit(np.array([stateAction]), np.array([target]), verbose = self.__verbose)
...
class NQLearningNetwork(HolographicNeuralNetwork):
    ...
    def __init__(self, nQlearning, hrrSize, hiddenLayers = [], learnRate = 1, discountFactor = 0.5, copyFrequency = 10, verbose = False):
    ...
    def policy(self, values, epsilon = 0.0):
        ...
        return np.argmax(np.average(values, axis=1))
    
    def update(self, state, prevPrediction, targetValues, action):
        toUpdate = random.randrange(self.__nQlearning)
        maxAction = np.argmax(targetValues[:,toUpdate])
        prevState = prevPrediction[0][action]
    ...
    def predict(self, state, actions, epsilon = 0.0, learn = False):
        ...
        prevPrediction = self.lastPrediction()
        ...
        if learn and prevPrediction:
            ...
            self.update(state, prevPrediction, targetValues, self.__prevAction)
        self.__prevAction = self.policy(values, epsilon)
